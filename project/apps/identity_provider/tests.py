...
class ApiKeyManagerTest(TestCase):
    def setUp(self):
        ...
        self.non_admin_user, _ = get_user_model().objects.get_or_create(
            username="non-admin",
            email="test@test.com",
            password="admin",
            is_superuser=False,
    ...
    def test_user_with_enterprise_sub(self):
        ...
        """
        self.sub_manager.create_company_subscription(
            self.enterprise_group, self.company
        )
    ...
    def test_user_with_free_sub(self):
        ...
        """
        self.sub_manager.create_individual_subscription(
            self.free_group, self.non_admin_user
        )
    ...
    def test_user_with_pro_sub(self):
        ...
        """
        self.sub_manager.create_individual_subscription(
            self.pro_group, self.non_admin_user
        )
        self.client.force_authenticate(user=self.non_admin_user)
        resp = self.client.post(reverse("api_key_manager"))
        self.assertEqual(403, resp.status_code)
    ...
    def test_user_with_enterprise_and_free_sub(self):
        ...
        self.sub_manager.create_company_subscription(
            self.enterprise_group, self.company_bar
        )
        self.client.force_authenticate(user=self.non_admin_user)
        resp = self.client.post(reverse("api_key_manager"))
        self.assertEqual(200, resp.status_code)
