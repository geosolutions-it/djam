...
class ApiKey(models.Model):
    user = models.ForeignKey(
        get_user_model(), null=False, blank=False, on_delete=models.CASCADE, help_text="API key will have the same privilege groups as it's owner."
    )
    key = models.UUIDField(null=False, blank=False, default=uuid4)
    revoked = models.BooleanField(null=False, default=False, help_text='If the API key is revoked, clients cannot use it.')
