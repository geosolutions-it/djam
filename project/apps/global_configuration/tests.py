...
from apps.global_configuration.models import GlobalConfiguration, SingletonRawOperationError
...
class SingletonModelCRUDTest(TestCase):
    ...
    def test_create_configuration_with_load(self):
        ...
        self.assertEquals(GlobalConfiguration.objects.all().count(), expected_models_count)
        GlobalConfiguration.load()
        self.assertEquals(GlobalConfiguration.objects.all().count(), expected_models_count)

    def test_create_configuration_with_create(self):
        self.assertRaises(SingletonRawOperationError, GlobalConfiguration.objects.create,
    ...
    def test_query_set_delete_configuration(self):
        GlobalConfiguration.load()
        self.assertRaises(SingletonRawOperationError, GlobalConfiguration.objects.all().delete)
