...
from apps.user_management.tasks import (
...
class EmailConfirmationSentView(View):
    def get(self, request):
        return render(
            request,
            "user_management/activation_msg_sent.html",
            context={
...
class EmailConfirmationView(View):
    def get(self, request, user_uuid, *args, **kwargs):
        ...
        try:
            ...
            logger.error(
                f"Email Confirmation View: No registered user found with user_uuid: {user_uuid}"
            )
            return self.render_error(request, "Whoops.. Something went wrong.")

        if user.email_confirmed:
            logging.info(
                f'Email Confirmation View: The user has already confirmed the email "{user.email}"'
            )
            return self.render_error(
                request, "Whoops.. Your email is already confirmed."
            )

        user_activation_code = UserActivationCode.objects.filter(
            activation_code=activation_code
        ).first()
        if user_activation_code is None:
            logging.error(
                f"Email Confirmation View: User Activation Code not found: {activation_code}"
            )
            return self.render_error(
                request,
                f'Whoops.. Something went wrong.<br><a href="{request.build_absolute_uri(reverse("resend_verification_email"))}">Resend an activation email</a>.',
            )

        if user_activation_code.user.id != user.id:
            logging.error(
                f"Email Confirmation View: User activation code mismatch for user: {user.email}"
            )
            return self.render_error(
                request,
                f'Whoops.. Something went wrong.<br><a href="{request.build_absolute_uri(reverse("resend_verification_email"))}">Resend an activation email</a>.',
        ...
        code_expired = timedelta(hours=settings.IP_ACTIVATION_CODE_EXPIRATION_HOURS) < (
            timezone.now() - user_activation_code.creation_date
        )

        if code_expired:
            logging.error(
                f"Email Confirmation View: User activation code expired for user: {user.email}"
            )
            return self.render_error(
                request,
                f'Whoops... Your activation code has expired.<br><a href="{request.build_absolute_uri(reverse("resend_verification_email"))}">Resend an activation email</a>.',
            )
    ...
    def render_error(self, request, error):
        return render(
            request, "user_management/simple_message.html", context={"error": error}
        )
...
class ResendVerificationEmailView(View):
    def get(self, request):
        ...
        return render(
            request,
            "user_management/signup_resend_activation_email.html",
            context={"form": form},
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        logger.info(
            f'Resend Email Verification View: User with email "{form.data["email"]}" requested resending activation message.'
        )
        ...
        if form.is_valid():
            ...
            if user is None:
                logger.error(
                    f"Resend Email Verification View: No registered user found with email: {user_email}"
                )
            ...
            if user.email_confirmed:
                logging.info(
                    f'Resend Email Verification View: The user has already confirmed the email "{user.email}"'
                )
                return self.render_error(
                    request,
                    "Your email is already confirmed.<br> No need for resending activation email :)",
                )
