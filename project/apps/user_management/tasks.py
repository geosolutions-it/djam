...
def send_user_notification_email(
    msg: str,
    receivers: typing.List,
    sender=settings.DEFAULT_FROM_EMAIL,
    subject="Djam notification",
):
    # Email subject *must not* contain newlines
    subject = "".join(subject.splitlines())
...
def send_staff_notification_email(
    msg, sender=settings.DEFAULT_FROM_EMAIL, subject="Djam administration notification"
):
    # Email subject *must not* contain newlines
    subject = "".join(subject.splitlines())
    ...
    msg = EmailMessage(
        ...
        body=msg,
        from_email=sender,
        to=[staff_member.email for staff_member in staff_members],
    )
    return msg.send()
