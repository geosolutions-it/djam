...
from apps.hubspot_integration.utils import (
    send_hubspot_notify,
...
def is_hubspot_configured():
    if not settings.REGISTRATION_EMAIL_CONFIRMATION or settings.REGISTRATION_MODERATION:
        logging.debug(
            "Hubspot registration: SKIPPED. Registration available only for Email Confirmation Registration only flow."
        )
        return False
...
def register_user_in_hubspot(sender, instance, **kwargs):
    ...
    try:
        ...
        logging.debug(
            f'Hubspot registration: SKIPPED. User with "{instance.email}" email does not exist in the DB yet.'
        )
        return

    if not db_user.email_confirmed and instance.email_confirmed:
        # user's email is confirmed - register the user in Hubspot, sending their subscription decision
        logging.info(
