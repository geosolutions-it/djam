...
class AccountManagementModel(AbstractBaseModel):
    ...
    user = models.ForeignKey(
        get_user_model(),
        blank=True,
        related_name="accountmodel_users",
        on_delete=models.DO_NOTHING,
    )
    company = models.ForeignKey(
        Company, max_length=250, blank=True, null=True, on_delete=models.DO_NOTHING
    )

    subscription_plan = models.CharField(
        max_length=250, choices=[(x, x) for x in SubscriptionPermissions.ALL]
    )
...
class CompanySubscription(Subscription):
    company = models.ForeignKey(
        Company, max_length=250, blank=False, null=True, on_delete=models.CASCADE
    )
