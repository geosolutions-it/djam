...
class CompanySubsForm(forms.ModelForm):
    groups = forms.ChoiceField(choices=[(x.lower(), x.lower()) for x in SubscriptionPermissions.COMPANY])
    def clean(self):
...
class IndividualSubsForm(forms.ModelForm):
    groups = forms.ChoiceField(choices=[(x.lower(), x.lower()) for x in SubscriptionPermissions.INDIVIDUAL])
    def clean(self):
        cleaned_data = super().clean()
        cleaned_data['groups'] = Group.objects.get(name=cleaned_data['groups'])
        try:
            subscription_manager.can_add_new_subscription_by_user(cleaned_data['user'], SubscriptionTypeEnum.INDIVIDUAL)
        except Exception as e:
            # TODO: add the list of the users to be checked
            raise forms.ValidationError("The selected user already have an active INDIVIDUAL subscription.")
